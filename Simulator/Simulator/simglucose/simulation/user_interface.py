...
def pick_insulin_pump(pump_selection=None):
    if pump_selection is None:
        ...
        pump_params = pd.read_csv(INSULIN_PUMP_PARA_FILE)
        selection = pump_selection
    pump = pump_params['Name'][selection - 1]
    logger.info('Selected Pumps:\n{}'.format(pump))
    return pump


def pick_scenario(selection=None, seed=None, start_time=None):
...
def build_envs(scenario, start_time, seed=None, cgm_selection=None, pump_selection=None):
    print("pump: ", pump_selection)
    patient_names = pick_patients()
    cgm_sensor_name, cgm_seed = pick_cgm_sensor(cgm_selection=cgm_selection, seed=seed)
    insulin_pump_name = pick_insulin_pump(pump_selection=pump_selection)
    if scenario is None:
...
def create_sim_instance(sim_time=None,
    ...
    if sim_time is None:
        ...
        sim_time = timedelta(hours=float(sim_time))

    if scenario is None:
        scenario = pick_scenario(selection=selection, seed=seed, start_time=start_time)

    envs = build_envs(scenario, start_time=start_time, seed=seed, cgm_selection=cgm_selection, pump_selection=pump_selection)

    if controller is None:
        controller = pick_controller()

    ctrllers = [controller for _ in range(len(envs))]# [copy.deepcopy(controller) for _ in range(len(envs))]
...
def simulate(sim_time=None,
    ...
    if save_path is None:
        save_path = pick_save_path(name)

    sim_instances = create_sim_instance(scenario=scenario,
